...
class FinetuningArguments(FreezeArguments, LoraArguments, RLHFArguments, GaloreArguments, BAdamArgument):
    ...
    """
    bert_learn: bool = field(
        default=False,
        metadata={"help": ("用于开启使用bert类型来进行训练")},
    )
    num_labels: int = field(
        default=3,
        metadata={"help": ("用于指定label的类别数")},
    )
    wandbproject: str = field(
        default="xinan_pretrain",
        metadata={"help": "Whether or not to train model in purely bf16 precision (without AMP)."},
    )
    wandbrun_name: str = field(
        default="darren_zyf",
        metadata={"help": "Whether or not to train model in purely bf16 precision (without AMP)."},
    )
    print_model_frozen: bool = field(
        default=False,
        metadata={"help": "打印哪些参数被冻结了"},
    )
